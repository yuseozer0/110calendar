...
export function PwaActions({ compact = false }: { compact?: boolean }) {
  ...
  // Keep an already-approved device registered after deployments or token refreshes.
  useEffect(() => {
    if (!app || !notificationSupported || notificationPermission !== 'granted' || !VAPID_KEY || !('serviceWorker' in navigator)) return
    let cancelled = false
    ...
    const syncSubscription = async () => {
      try {
        ...
        console.warn('[push] subscription sync skipped:', syncError)
      }
    }
  ...
  }, [app, notificationPermission, notificationSupported])
